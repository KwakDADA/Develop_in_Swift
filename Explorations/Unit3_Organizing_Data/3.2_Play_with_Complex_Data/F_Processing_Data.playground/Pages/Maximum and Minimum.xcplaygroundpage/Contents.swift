...
func indexOfMaximum(from tabulatedValues: [TabulatedValue]) -> Int {
    ...
    for i in 0..<tabulatedValues.count - 1 {
        if tabulatedValues[i] > tabulatedValues[maxIdx] {
            maxIdx = i
        }
    }
    return maxIdx
...
func indexOfMinimum(from tabulatedValues: [TabulatedValue]) -> Int {
    var minIdx = 0
    for i in 0..<tabulatedValues.count - 1 {
        if tabulatedValues[i] < tabulatedValues[minIdx] {
            minIdx = i
        }
    }
    return minIdx
...
for _ in 1...3 {
    ...
    for i in 0..<allShows.count {
        if allShows[i] > allShows[maxIdx] {
            maxIdx = i
        }
    }
    mostPopular.append(allShows.remove(at: maxIdx))
}
...
for _ in 1...3 {
    var minIdx = 0
    for i in 0..<allShows.count {
        if allShows[i] < allShows[minIdx] {
            minIdx = i
        }
    }
    leastPopular.append(allShows.remove(at: minIdx))
}
