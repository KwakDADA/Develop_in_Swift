...
func legs() {
    display.batchSetPixels(block(x: 16, y: 12, width: 2, height: 4, color: .blue))
}
func torso() {
    display.batchSetPixels(block(x: 15, y: 16, width: 4, height: 3, color: .white))
}
func head() {
    display.batchSetPixels(block(x: 16, y: 19, width: 2, height: 2, color: .yellow))
}
