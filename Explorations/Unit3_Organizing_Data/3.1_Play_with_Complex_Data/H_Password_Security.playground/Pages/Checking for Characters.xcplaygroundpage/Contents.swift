...
func password(_ password: String) {
    guard !tenCommonPasswords.contains(password) else {
        print("Please recreate the password that cannot be easily inferred.")
        return
    }
    guard password.filter({digits.contains($0)}).count > 0 else {
        print("Please include at least one digit")
        return
    }
    guard password.filter({punctuation.contains($0)}).count > 0 else {
        print("Please include at least one punctuation character.")
        return
    }
