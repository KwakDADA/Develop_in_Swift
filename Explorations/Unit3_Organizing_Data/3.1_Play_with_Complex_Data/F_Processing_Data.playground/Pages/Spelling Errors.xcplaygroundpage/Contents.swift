...
func closestMatch(for string: String, from potentialMatches: [String]) -> String {
    ...
    for i in 0 ..< potentialMatches.count {
        ...
        let ed = editDistance(from: string, to: potentialMatch)
...
var lowercaseSurvey = [String]()
for response in surveyData {
    lowercaseSurvey.append(response.lowercased())
}
for response in lowercaseSurvey {
    if lowercaseCatalog.contains(response) {
        votes.incrementCount(forValue: response)
    } else {
        let maybe = closestMatch(for: response, from: lowercaseCatalog)
        votes.incrementCount(forValue: maybe)
    }
}

// Loop through all tabulator values. Print only those that are contained in the lowercase version of the show catalog.
for vote in votes.values {
    if lowercaseCatalog.contains(vote) {
        print(vote)
    }
}
// Print a header
...
for vote in votes.values {
    if lowercaseCatalog.contains(vote) {
        print("\(vote): \(votes.count(forValue: vote))")
    } else {
        error += 1
    }
}
